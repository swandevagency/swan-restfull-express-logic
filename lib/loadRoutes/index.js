                ...
                if (!directory.split('.')[1]) {
                    loadRoutes(directory).forEach((route) => {
                        if (!fs.existsSync(`${apisDirectory}/${directory}/plugin.json`)) {
                            loadRoute(directory, route, null);
                        }else{
                            if (!fs.existsSync(`${apisDirectory}/${directory}/controllers/${route}.js`)) {
                                const {name} = require(`${apisDirectory}/${directory}/plugin.json`);
                                if (!name) {
                                    reject("Something went wrone !")
                                }
                                loadRoute(directory, route, name);
                            }else{
                                loadRoute(directory, route, null);
                            }
